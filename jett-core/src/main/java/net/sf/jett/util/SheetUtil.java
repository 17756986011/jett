...
public class SheetUtil {
    ...
    private static void copyColumnWidthsLeft(Sheet sheet, int colStart, int colEnd, int numCols) {
        ...
        for (int colNum = colStart; colNum <= colEnd; colNum++) {
            ...
            logger.info("    Setting column width on col {} to col {}'s width: {}.",
    ...
    private static void copyRowHeightsUp(Sheet sheet, int rowStart, int rowEnd, int numRows) {
        ...
        for (int rowNum = rowStart; rowNum <= rowEnd; rowNum++) {
            ...
            if (row == null && newRow != null) {
                ...
                logger.info("      Setting row height on row {} to \"standard\" row height of {}. (Row {} does not exist.)",
                        newRowNum, newRow.getHeight(), rowNum);
            } else if (row != null) {
                if (newRow == null) {
                    newRow = sheet.createRow(newRowNum);
                }
                logger.info("      Setting row height on row {} to row {}'s height: {}",
                ...
                for (int colIndex = colStart; colIndex <= colEnd; colIndex++) {
                    ...
                    if (cell == null && newCell != null) {
                        ...
                        if (origCellRef != null) {
                            ...
                            logger.info("sCL: Replacing {} => {} with {} => {}",
                ...
                for (int colIndex = colEnd; colIndex >= colStart; colIndex--) {
                    ...
                    if (cell == null && newCell != null) {
                        ...
                        if (origCellRef != null) {
                            tagLocationsMap.remove(cellRef);
                            tagLocationsMap.put(newCellRef, origCellRef);
                            logger.info("sCR: Replacing {} => {} with {} => {}",
                                    cellRef, origCellRef, newCellRef, origCellRef);
                        }
            ...
            for (int rowIndex = rowStart; rowIndex <= rowEnd; rowIndex++) {
                ...
                if (cell == null && newRow != null && newCell != null) {
                    ...
                    if (origCellRef != null) {
                        tagLocationsMap.remove(cellRef);
                        tagLocationsMap.put(newCellRef, origCellRef);
                        logger.info("sCU: Replacing {} => {} with {} => {}",
                                cellRef, origCellRef, newCellRef, origCellRef);
                    }
            ...
            for (int colIndex = colStart; colIndex <= colEnd; colIndex++) {
                ...
                if (cell == null && newCell != null) {
                    ...
                    if (origCellRef != null) {
                        ...
                        logger.info("sCD: Replacing {} => {} with {} => {}",
                                cellRef, origCellRef, newCellRef, origCellRef);
                    }
                }
        ...
        if (add) {
            ...
            for (int i = 0; i < numMergedRegions; i++) {
                ...
                if (isCellAddressWhollyContained(region, left, right, top, bottom)) {
                    ...
                    if (remove) {
                        logger.info("      Updating adjusted merged region from {} to {}.", region, newRegion);
                        sheetMergedRegions.set(i, newRegion);
                    } else {
                        logger.info("      Copying merged region from {} to {}.", region, newRegion);
            ...
            for (int i = 0; i < numMergedRegions; i++)
                for (CellRangeAddress region : sheetMergedRegions) {
                    if (isCellAddressWhollyContained(region, left, right, top, bottom)) {
                        logger.info("      Removing merged region: {}");
...
//            if (!keep[j])
//            {
//               logger.info("      Removing Conditional Formatting at region: {}");
//            }
//         }
//
...
//
//               logger.info("      Shifting Conditional Formatting at region: {} to: {}", region, shifted);
//
//               regions[j] = shifted;
//            }
//         }
...
//
//               logger.info("      Copying Conditional Formatting at region: {} to {}", region, copy);
//            }
//         }
//
...
//   }
            ...
            if (r != null) {
                for (int cellNum = left; cellNum <= right; cellNum++) {
                    ...
                    if (c != null) {
                        ...
                        logger.info("dB: Removing {}", cellRef);
                    }
                }
            }
        }
            ...
            if (r != null) {
                for (int cellNum = left; cellNum <= right; cellNum++) {
                    ...
                    if (c != null) {
                        ...
                        c.removeHyperlink();
                        tagLocationsMap.remove(cellRef);
                        logger.info("cB: Removing {}", cellRef);
                    }
                }
            }
        }
                    ...
                    if (cell.getCellType() == Cell.CELL_TYPE_STRING) {
                        ...
                        while (exprBegin != -1 && exprEnd != -1 && exprEnd > exprBegin) {
                            ...
                            if (replaceExpr) {
                                ...
                                logger.info("    cellText is now \"{}\".", cellText);
        ...
        switch (block.getDirection()) {
            case VERTICAL:
                ...
                if (ancestor.getParent() == null &&
                    ...
                    logger.info("  Shrinking ancestor block ({}) by {} rows!",
            ...
            case HORIZONTAL:
                ...
                if (ancestor.getParent() == null &&
                    ...
                    logger.info("  Shrinking ancestor block ({}) by {} columns!",
        ...
        switch (dir) {
            case VERTICAL:
                while (ancestor != null) {
                    ...
                    if (numVertCells != 0) {
                        logger.info("    Growing ancestor block ({}) by {} rows!", ancestor, numVertCells);
                        ancestor.expand(0, numVertCells);
                    }
                    if (numHorizCells != 0) {
                        logger.info("    Growing ancestor block ({}) by {} columns!", ancestor, numHorizCells);
        ...
        switch (block.getDirection()) {
            case VERTICAL:
                ...
                while (translateDown > 0) {
                    ...
                    logger.info("    emptyRowsAtBottom: {}", emptyRowsAtBottom);

                    Block toShift = new Block(null, startCellNum, endCellNum, startRowNum, endRowNum);
                    logger.info("    Block to shift: {} by {} rows.", toShift, translateDown);
                    ...
                    if (translateDown > 0) {
                        logger.info("    Growing ancestor block ({}) by {} rows!", ancestor, translateDown);
            ...
            case HORIZONTAL:
                ...
                while (translateRight > 0) {
                    ...
                    logger.info("    emptyColsAtRight: {}", emptyColsAtRight);

                    Block toShift = new Block(null, startCellNum, endCellNum, startRowNum, endRowNum);
                    logger.info("    Block to shift: {} by {} columns.", toShift, translateRight);
                    blocksToShift.push(toShift);
                    ...
                    if (translateRight > 0) {
                        ...
                        logger.info("    Growing ancestor block ({}) by {} columns!", ancestor, translateRight);
    ...
    public static Block copyBlock(Sheet sheet, TagContext tagContext, Block block, WorkbookContext context, int numBlocksAway) {
        ...
        switch (block.getDirection()) {
            case VERTICAL:
                logger.trace("  Case Vertical");
                // Copy Cells.
                logger.info("    Copying cells {} to {}, row {} to {} by {} rows.",
                ...
                for (int r = top; r <= bottom; r++) {
                    ...
                    for (int c = left; c <= right; c++) {
                        ...
                        if (origCellRef != null) {
                            tagLocationsMap.put(newCellRef, origCellRef);
                            logger.info("cB: Adding {} => {}", newCellRef, origCellRef);
                        }
            ...
            case HORIZONTAL:
                ...
                // Copy Cells.
                logger.info("    Copying cells {} to {}, row {} to {} by {} columns.",
    ...
    public static void groupColumns(Sheet sheet, int begin, int end, boolean collapse) {
        ...
        if (sheet instanceof XSSFSheet) {
            logger.info("Def. col width = {}", sheet.getDefaultColumnWidth());
        ...
        if (sheet instanceof XSSFSheet) {
            ...
            for (int c = begin; c <= end; c++) {
                ...
                logger.info("Setting Col {}, to width {}", c, w);
        ...
        if (collapse) {
            if (sheet instanceof XSSFSheet) {
                ...
                for (int c = begin; c <= end; c++) {
                    logger.info("Setting Col {} hidden", c);
    ...
    public static Color getColor(Workbook workbook, String value) {
        ...
        } else // XSSFWorkbook
        {
            ...
            if (value.startsWith("#") && value.length() == 7) {
                ...
                logger.info("  XSSFColor created: {}", color);
            } else {
                ...
                try {
                    ...
                    logger.info("  XSSFColor name matched: {}", value);
                } catch (IllegalArgumentException e) {
                    logger.info("  XSSFColor name not matched ({}): {}", value, e.toString());
                }
            }
        }
        return color;
